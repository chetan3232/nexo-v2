...
export const ChatPanel: React.FC<ChatPanelProps> = ({ onSend }) => {
  ...
  const { events, isGenerating } = useAgentEventStore();
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Track which file paths are fully done (for FileCreateCard state)
  const [doneFiles, setDoneFiles] = useState<Set<string>>(new Set());
  const [fileChunks, setFileChunks] = useState<Record<string, string>>({});

  // Update doneFiles and fileChunks from events
  useEffect(() => {
    const done = new Set<string>();
    const chunks: Record<string, string> = {};
    events.forEach((e) => {
      if (e.type === "file_done") done.add(e.path);
      if (e.type === "file_write") chunks[e.path] = e.chunk;
    });
    setDoneFiles(done);
  ...
  // Decide what to render: if we have agent events (new session), use event timeline.
  // Otherwise fall back to legacy messages for old chat history.
    ...
    <div className="h-full flex flex-col bg-stone-50/30 backdrop-blur-xl border-r border-stone-200 relative">
      ...
      <div className="h-12 border-b border-stone-200 flex items-center justify-between px-4 shrink-0 bg-white/50 backdrop-blur-md z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm text-stone-900 tracking-tight">Nexo</span>
          {isGenerating && (
      ...
      {/* Content Area */}
      <div className="flex-1 overflow-y-auto p-4 pb-36 space-y-3 bg-white no-scrollbar">
        <AnimatePresence initial={false}>
          {hasAgentEvents ? (
            // 🔥 NEW: Live Agent Event Timeline
            events.map((event, i) => {
              // Deduplicate: skip file_write cards if we already rendered one for this path
              if (event.type === "file_write") {
                const prevWrite = events.slice(0, i).find(
                  (e) => e.type === "file_write" && (e as any).path === event.path
                );
                if (prevWrite) return null; // only show first write event per file
              }
              return (
                <EventRenderer
                  key={`${event.type}-${i}`}
            ...
            // Fallback: legacy message bubbles for existing chat history
            messages.map((msg, i) => (
              <motion.div
              ...
              >
                <div className={`px-5 py-3 rounded-2xl text-[13px] max-w-[90%] leading-relaxed shadow-sm border ${msg.role === "user" ? "bg-stone-900 text-white border-stone-800" : "bg-white text-stone-800 border-stone-200"}`}>
                  {msg.role === "assistant" && (
          ...
          {/* Building indicator when no events yet */}
          {buildPhase === "building" && !hasAgentEvents && (
            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
              <div className="bg-white border border-stone-100 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-3 w-fit">
                ...
                <span className="text-[10px] font-black text-stone-900 uppercase tracking-widest flex items-center gap-2">
                  Nexo AI: <span className="text-stone-400 font-bold normal-case tracking-normal">{subStatus || "Processing..."}</span>
                </span>
              </div>
            </motion.div>
          )}
      ...
      {/* Input Area */}
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-stone-100">
        <div className={`relative border rounded-2xl bg-white transition-all duration-300 overflow-hidden flex flex-col ${buildPhase === "building" ? "border-indigo-400 ring-2 ring-indigo-100 shadow-lg shadow-indigo-50/50" : "border-stone-200 shadow-sm focus-within:border-stone-300 focus-within:shadow-md"}`}>
          <textarea
            value={input}
            ...
            placeholder={
              buildPhase === "building"
                ? "AI is building your app... please wait ✨"
          ...
          <div className="flex items-center justify-between px-2 pb-2">
            ...
            <button
              ...
              className="p-2 bg-stone-900 text-white rounded-full hover:bg-black disabled:opacity-40 transition-all flex items-center gap-1.5 px-4 text-xs font-bold"
            >
              {buildPhase === "building" ? (
