import { FileManifest, EditType, EditIntent, IntentPattern } from "../types";
...
export interface ProjectIntentValidation {
  valid: boolean;
  status: "VALID" | "MODE_CONFLICT";
  adaptedPrompt: string;
  adaptedProjectType: string;
  adaptedTechs: string[];
  reason?: string;
...
/**
 * Validates the user's project intent against the selected project mode and adjusts it if there's a conflict.
 */
export function validateProjectIntent(
  prompt: string,
  projectMode: "frontend" | "fullstack",
  projectType?: string,
  requestedTechs?: string[]
): ProjectIntentValidation {
  const lowerPrompt = prompt.toLowerCase();
  const techs = requestedTechs ? requestedTechs.map(t => t.toLowerCase()) : [];

  const forbiddenFrontendTechs = ["react", "typescript", "node.js", "node", "express", "python", "php", "java", "vue", "angular", "next.js", "nextjs"];
  const fullstackConcepts = ["database", "backend", "api", "mongodb", "sql", "postgres", "mysql", "server"];

  if (projectMode === "frontend") {
    const hasForbiddenTech = forbiddenFrontendTechs.some(tech =>
      lowerPrompt.includes(tech) || techs.includes(tech)
    );

    const hasFullstackConcepts = fullstackConcepts.some(concept =>
      lowerPrompt.includes(concept)
    );

    const isEcomApp = lowerPrompt.includes("e-commerce") || lowerPrompt.includes("ecommerce") || lowerPrompt.includes("shopping cart") || lowerPrompt.includes("store");
    const isDashboardOrSaaS = lowerPrompt.includes("dashboard") || lowerPrompt.includes("saas");

    if (hasForbiddenTech || hasFullstackConcepts || isEcomApp || isDashboardOrSaaS) {
      let adaptedPrompt = prompt;
      let reason = "";

      if (isDashboardOrSaaS) {
        adaptedPrompt = "A landing-page-style SaaS product website using HTML, CSS and JavaScript.";
        reason = "SaaS dashboard requests are adapted to landing pages in Frontend mode.";
      } else if (isEcomApp) {
        adaptedPrompt = "An e-commerce landing page using HTML, CSS and JavaScript.";
        reason = "E-commerce applications are adapted to landing pages in Frontend mode.";
      } else {
        adaptedPrompt = prompt
          .replace(/\busing react\b/gi, "")
          .replace(/\breact\b/gi, "")
          .replace(/\btypescript\b/gi, "JavaScript")
          .replace(/\bnext\.js\b/gi, "")
          .replace(/\bvue\b/gi, "")
          .replace(/\bangular\b/gi, "")
          .replace(/\s+/g, " ")
          .trim();
        adaptedPrompt += " using HTML, CSS and JavaScript.";
        reason = "Frontend mode does not support the requested framework or full-stack technologies.";
      }
      ...
      return {
        ...
        adaptedPrompt,
        adaptedProjectType: "landing-page",
        adaptedTechs: ["HTML5", "CSS3", "Vanilla JavaScript"],
        reason,
        generateFiles: ["index.html", "style.css", "script.js"]
      };
    }
    ...
    return {
      ...
      status: "VALID",
      adaptedPrompt: prompt,
      adaptedProjectType: projectType || (lowerPrompt.includes("portfolio") ? "portfolio" : "landing-page"),
      adaptedTechs: ["HTML5", "CSS3", "Vanilla JavaScript"],
      generateFiles: ["index.html", "style.css", "script.js"]
    };
  } else {
    // Fullstack Mode
    let adaptedPrompt = prompt;
    let adaptedProjectType = projectType || "fullstack-application";
    const adaptedTechs = ["React", "TypeScript", "Node.js"];

    if (lowerPrompt.includes("portfolio")) {
      adaptedPrompt = "Create a full-stack portfolio application using React, TypeScript, and Node.js.";
      adaptedProjectType = "fullstack-application";
    }
